...
function normalizePlan(value: unknown): Plan | null {
  ...
  if (!id || !creatorId || !title || !createdAt) return null;

  const slotsRaw = Array.isArray(row.suggested_slots) ? row.suggested_slots : [];
  const suggestedSlots = slotsRaw.filter((item): item is string => typeof item === "string");
  ...
  return {
    ...
    chosen_slot: typeof row.chosen_slot === "string" ? row.chosen_slot : null,
...
export default function PlansList() {
  ...
  const fetchUserAndPlans = useCallback(async () => {
    const {
      data: { user },
    ...
    if (data) {
      const nextPlans = data.map(normalizePlan).filter((item): item is Plan => item !== null);
      setPlans(sortPlans(nextPlans));
    }
  }, [supabase]);

  useEffect(() => {
    void fetchUserAndPlans();
    ...
    const handlePlanCreated = (event: Event) => {
      ...
      const incoming = normalizePlan(customEvent.detail);
      if (!incoming) return;
      setPlans((prev) => {
        if (prev.some((item) => item.id === incoming.id)) return prev;
        return sortPlans([incoming, ...prev]);
      });
    };

    window.addEventListener("plans:created", handlePlanCreated as EventListener);

    return () => {
      window.removeEventListener("plans:created", handlePlanCreated as EventListener);
      void supabase.removeChannel(channel);
    };
  }, [fetchUserAndPlans, supabase]);

  const handlePickSlot = async (planId: string, slot: string) => {
    hapticFeedback.light();
    const toastId = toast.loading("Подтверждаем...");
    const result = await pickPlanSlotAction(planId, slot);
    if (result.error) {
  ...
  const handleRemind = async (planId: string) => {
    const toastId = toast.loading("Отправляем напоминание...");
    const result = await sendPlanReminderAction(planId);
    if (result.error) {
      toast.error(result.error, { id: toastId });
      return;
    }
  ...
  const handleDelete = async (plan: Plan) => {
    if (deletingIds[plan.id]) return;
    if (!window.confirm("Точно удалить этот план?")) return;

    setDeletingIds((prev) => ({ ...prev, [plan.id]: true }));
    ...
    try {
      ...
      toast.success("План удалён", { id: toastId });
    } catch {
      setPlans(previous);
      toast.error("Не удалось удалить план", { id: toastId });
    } finally {
      setDeletingIds((prev) => {
        const next = { ...prev };
        delete next[plan.id];
    ...
    return (
      <div className="w-full flex flex-col items-center justify-center py-16 text-center opacity-70">
        ...
        <p className="text-sm text-[#4a403b]/80 dark:text-[#d4c8c1]/80 mt-1">
          Самое время запланировать что-то особенное! ✨
        </p>
      </div>
    );
  }
      ...
      <div className="flex bg-[#e8dfd5] dark:bg-[#3d332c] p-1 rounded-2xl w-full mb-2">
        <button
          ...
          className={`flex-1 py-2 rounded-xl font-bold transition-colors ${
            viewMode === "list"
              ? "bg-[#fdfbf9] dark:bg-[#1a1614] text-[#9e6b36] dark:text-[#cca573] shadow-sm"
              : "opacity-60 text-current hover:opacity-100"
          }`}
        >
          Список
        </button>
      ...
      {viewMode === "calendar" ? (
        <div className="w-full bg-[#fdfbf9] dark:bg-[#2c2623] p-4 rounded-3xl border border-[#e8dfd5] dark:border-[#3d332c] shadow-sm animate-in fade-in duration-300">
          <div className="flex justify-between items-center mb-4 px-2">
            <button
              onClick={prevMonth}
              className="px-3 py-1 bg-black/5 dark:bg-white/10 rounded-lg hover:bg-[#cca573] hover:text-white transition-colors"
            >
              ◀
            </button>
          ...
          <div className="grid grid-cols-7 gap-1 text-center mb-2">
            {["Пн", "Вт", "Ср", "Чт", "Пт", "Сб", "Вс"].map((day) => (
              <div key={day} className="font-bold text-xs opacity-50">
                {day}
              </div>
            ))}
          </div>
          <div className="grid grid-cols-7 gap-1">
            {days.map((day) => {
              ...
              const dayPlans = plans.filter((plan) => plan.target_date && isSameDay(parseISO(plan.target_date), day));
              const isToday = isSameDay(day, new Date());

              return (
                <div
                  key={day.toString()}
                  className={`aspect-square flex flex-col items-center justify-start py-2 px-1 rounded-xl border transition-all ${
                    isCurrentMonth
                      ? "bg-white dark:bg-[#1f1a16] border-[#e8dfd5] dark:border-[#3d332c]"
                      : "bg-transparent border-transparent opacity-30"
                  } ${isToday ? "border-[#cca573] border-2 shadow-sm" : ""}`}
                >
                  <span className={`text-sm font-bold ${isCurrentMonth ? "" : "opacity-40"} ${isToday ? "text-[#e07a5f]" : ""}`}>
                    {format(day, "d")}
                  </span>
                  <div className="flex flex-wrap gap-1 mt-1 justify-center w-full">
                    {dayPlans.map((plan) => (
                      <div
                        key={plan.id}
                        className={`w-2.5 h-2.5 rounded-full shadow-sm ${plan.status === "locked" ? "bg-[#81b29a]" : "bg-[#e07a5f]"}`}
                        title={plan.title}
                      />
                    ))}
                  </div>
        ...
        <>
          {plans.map((plan) => {
            ...
            return (
              <div
                key={plan.id}
                className={`p-5 rounded-3xl border-2 transition-all flex flex-col gap-3 ${
                  isLocked
                    ? "bg-[#f0f9f0] dark:bg-[#203320] border-[#a3d9a3] opacity-90"
                    : "bg-white dark:bg-[#2d2621] border-[#e3d2b3] dark:border-[#55331a]"
                }`}
              >
                ...
                {plan.description && <p className="text-sm opacity-80 whitespace-pre-wrap">{plan.description}</p>}

                {!isLocked && plan.suggested_slots.length > 0 && (
                  <div className="mt-2 flex flex-col gap-2">
                    ...
                    {isCreator && (
                      <p className="text-xs text-[#9e6b36] mt-1 italic">≈ Ждём, пока {resolvedPartnerName} выберет удобный вариант</p>
                    )}
                  </div>
                )}
                ...
                <div className="flex justify-between items-center mt-3">
                  ...
                  {isCreator && (
                    <button
                      ...
                      disabled={deletingIds[plan.id]}
                      className="text-xs font-bold text-red-400 hover:text-red-500 opacity-70 hover:opacity-100 ml-auto disabled:opacity-40"
                    >
                      {deletingIds[plan.id] ? "Удаляем..." : "🗑️ Удалить"}
                    </button>
                  )}
                </div>
              </div>
