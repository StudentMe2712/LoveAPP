...
export async function createPlanAction(formData: FormData) {
  try {
    ...
    const suggestedSlots = rawSlots.map((slot) => slot.toString().trim()).filter(Boolean);
    ...
    const { data: plan, error } = await supabase
      ...
      .select("id,creator_id,title,description,suggested_slots,chosen_slot,status,target_date,created_at")
      .single<PlanRow>();
